...
function home(identity) {
  ...
  <section class="approach paper" id="approach"><picture class="chapter-vignette chapter-deer" aria-hidden="true"><source media="(max-width: 760px)" srcset="/assets/clean/chapter-glen-mobile.jpg"><img src="/assets/clean/chapter-glen-desktop.jpg" width="1942" height="809" loading="lazy" alt=""></picture><img class="floating-accent floating-sprig approach-sprig" src="/assets/clean/floating-sprig.png" width="320" height="320" loading="lazy" alt="" aria-hidden="true"><div class="shell"><p class="kicker">How I work</p><h2>I bring <em>context, creativity,</em> and <em>care</em> to every project.</h2><p class="lede">I take time to understand the business, the people, and the constraints. Then I craft technology solutions that make sense. Solutions that are sustainable, budget-conscious, and built to support growth and profitability.</p><div class="steps" data-accordion>${processStep(1,'understand','Understand deeply','I listen first and learn the business, users, market, and constraints before shaping a solution.')}${processStep(2,'creative','Solve creatively','I design thoughtful solutions that are pragmatic, elegant, and genuinely fit for purpose.')}${processStep(3,'constraints','Respect reality','I use right-sized teams and plan around real budgets, timelines, complexity, and support needs.')}${processStep(4,'growth','Build for lasting results','I focus on profit and efficiency while creating a roadmap that can adapt and grow with you.')}</div></div><a class="section-cue" href="#selected-work" aria-label="Continue to selected work"><span>↓</span></a></section>
  <section class="selected" id="selected-work"><picture class="chapter-vignette chapter-owl" aria-hidden="true"><source media="(max-width: 760px)" srcset="/assets/clean/chapter-watchtower-mobile.jpg"><img src="/assets/clean/chapter-watchtower-desktop.jpg" width="1942" height="809" loading="lazy" alt=""></picture><img class="floating-accent floating-firefly" src="/assets/clean/floating-firefly.png" width="320" height="320" loading="lazy" alt="" aria-hidden="true"><div class="shell selected-heading"><p class="kicker">Selected work</p><h2>A few projects I’m proud of.</h2></div><div class="project-slider" id="project-slider" data-slider aria-label="Selected projects">${Object.entries(projects).map(([slug, p]) => projectCard(slug, p)).join('')}</div>${railControls('project-slider','selected projects')}<a class="more" href="/work/"><span>View all projects</span><b aria-hidden="true">→</b></a><a class="section-cue" href="#about" aria-label="Continue to about me"><span>↓</span></a></section>
  <section class="about paper" id="about"><img class="chapter-vignette chapter-columbia" src="/assets/clean/chapter-columbia.png" width="1400" height="664" loading="lazy" alt="" aria-hidden="true"><img class="floating-accent floating-sprig about-sprig" src="/assets/clean/floating-sprig.png" width="320" height="320" loading="lazy" alt="" aria-hidden="true"><div class="shell about-grid"><div><p class="kicker">A little about me</p><h2><span>I’m a thought partner,</span><span>not just a developer.</span></h2><p>The best software solves the right problem in the right way. I bring a blend of technical expertise, business acumen, and creative thinking to every project. I care about clarity, communication, and building lasting partnerships.</p><div class="values" data-accordion>${aboutValue('about-teacup','Empathetic & curious','I listen deeply and ask the right questions.')}${aboutValue('about-lantern','Clear & collaborative','I keep things transparent and move as a team.')}${aboutValue('about-flower','Practical & grounded','I ship solutions that make sense today and scale tomorrow.')}</div></div></div><a class="section-cue" href="#contact" aria-label="Continue to contact"><span>↓</span></a></section>
...
function processStep(number, image, title, copy) {
  return `<details open><summary><span class="step-number">${String(number).padStart(2, '0')}</span><img src="/assets/clean/${image}.png" width="116" height="82" loading="lazy" alt=""><h3>${title}</h3><span class="accordion-mark" aria-hidden="true"></span></summary><p>${copy}</p></details>`;
}

function aboutValue(image, title, copy) {
  return `<details open><summary><img src="/assets/clean/${image}.png" width="70" height="77" loading="lazy" alt=""><h3>${title}</h3><span class="accordion-mark" aria-hidden="true"></span></summary><p>${copy}</p></details>`;
}

function contactChapter(identity, showCue = false) {
  return `<section class="home-contact contact-chapter" id="contact"><div class="shell home-contact-copy"><p class="kicker">Let’s connect</p><h2>Let’s grow something meaningful.</h2><p>I’m always open to thoughtful conversations and new opportunities. Have a project in mind, or just want to compare notes? I’d love to hear from you.</p><div class="home-contact-links"><a href="mailto:${identity.email}"><span class="icon">${icon("mail")}</span><span><small>Email</small><span class="contact-detail">${identity.email}</span></span><b aria-hidden="true">→</b></a><a href="https://linkedin.com/in/${identity.linkedin}"><span class="icon">${icon("linkedin")}</span><span><small>LinkedIn</small><span class="contact-detail">linkedin.com/in/${identity.linkedin}</span></span><b aria-hidden="true">→</b></a><a href="https://github.com/${identity.github}"><span class="icon">${icon("github")}</span><span><small>GitHub</small><span class="contact-detail">github.com/${identity.github}</span></span><b aria-hidden="true">→</b></a></div></div><img class="garden-bee" src="/assets/clean/garden-bee.png" width="320" height="320" loading="lazy" alt="" aria-hidden="true"><picture class="contact-garden" aria-hidden="true"><source media="(max-width: 760px)" srcset="/assets/clean/contact-garden-mobile.jpg"><img src="/assets/clean/contact-garden-desktop.jpg" width="2018" height="779" loading="lazy" alt=""></picture>${showCue ? '<a class="section-cue" href="#site-footer" aria-label="Continue to the footer"><span>↓</span></a>' : ''}</section>`;
}
